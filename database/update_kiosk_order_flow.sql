...
ALTER TABLE `orders`
  ...
  ADD COLUMN IF NOT EXISTS `payment_method` ENUM('debit','cashier') NOT NULL DEFAULT 'cashier' AFTER `notes`,
  ADD COLUMN IF NOT EXISTS `status` VARCHAR(255) NOT NULL DEFAULT 'pending' AFTER `payment_method`,
